...
<?php
try {
    $tc_stmt = $pdo->query("SELECT * FROM account_tiers ORDER BY price ASC");
    $tc_tiers = $tc_stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) { $tc_tiers = []; }
?>
<h3 style="font-size:1.1rem; font-weight:700; margin-top:2rem; margin-bottom:0.75rem;">21. ACCOUNT TIERS & SELLER RULES</h3>
<p>Users may choose from <?= count($tc_tiers) ?: 3 ?> account levels, each with specific system-enforced limits and benefits:</p>
<div style="display:flex; flex-direction:column; gap:1rem; margin-bottom:1.5rem;">
<?php foreach ($tc_tiers as $tc_tier): ?>
  <div style="padding:15px; border:1px solid <?= htmlspecialchars($tc_tier['badge']) ?>40; border-radius:12px; border-left:4px solid <?= htmlspecialchars($tc_tier['badge']) ?>; background:rgba(0,0,0,0.02);">
    <h4 style="font-size:1rem; font-weight:800; margin-bottom:0.5rem; text-transform:capitalize;"><?= htmlspecialchars($tc_tier['tier_name']) ?> Account</h4>
    <p style="margin:0; font-size:0.9rem; line-height:1.6;">
        The <strong><?= ucfirst($tc_tier['tier_name']) ?></strong> account is designed for <?= $tc_tier['tier_name'] === 'basic' ? 'casual sellers' : 'serious businesses' ?>.
        <?= $tc_tier['price'] <= 0 ? 'It is completely <strong>free</strong> and valid for ' . htmlspecialchars($tc_tier['duration']) . ' month' . ($tc_tier['duration'] == 1 ? '' : 's') . '.' : 'It requires a fee of <strong>₵' . number_format($tc_tier['price'], 2) . '</strong> and is valid for ' . htmlspecialchars($tc_tier['duration']) . ' month' . ($tc_tier['duration'] == 1 ? '' : 's') . '.' ?>
    </p>
    <?php $tc_bens = json_decode($tc_tier['benefits'] ?? '[]', true) ?: []; if(count($tc_bens) > 0): ?>
    <ul style="margin-top:0.75rem; margin-bottom:0.25rem; font-size:0.9rem; padding-left:1.5rem;">
        <?php foreach($tc_bens as $b): ?><li style="margin-bottom:0.25rem;"><strong><?= htmlspecialchars($b) ?></strong></li><?php endforeach; ?>
    </ul>
    <?php endif; ?>
  </div>
<?php endforeach; ?>
</div>
<p><strong>Admin Rights:</strong> Admin may adjust product limits, fees, badges, and ads boost benefits at any time.</p>
