...
$localPath = Join-Path $repoRoot 'alwaysdata.env'
...
$remoteEnvUrl = $ftpBase.TrimEnd('/') + '/.env'

Write-Host 'Uploading production .env ...' -ForegroundColor Cyan
& curl.exe --noproxy "*" --ssl-reqd -T $localPath $remoteEnvUrl -u $credential
if ($LASTEXITCODE -ne 0) {
    throw 'Upload failed for production .env'
}
...
Write-Host 'Production Google env deployment completed.' -ForegroundColor Green
