...
if ($search) {
    ...
    $stmt = $pdo->prepare("SELECT p.* FROM products p $p_join WHERE p.status='approved' AND u.vacation_mode = 0 AND (p.title LIKE ? OR p.category LIKE ? OR p.description LIKE ?) $p_order");
    $stmt->execute(["%$search%", "%$search%", "%$search%"]);
    $products = $stmt->fetchAll();
    
    // TYPO-TOLERANT FALLBACK
    if (count($products) === 0 && strlen($search) > 3) {
        $stmt = $pdo->prepare("SELECT p.* FROM products p $p_join WHERE p.status='approved' AND u.vacation_mode = 0 AND (SOUNDEX(p.title) = SOUNDEX(?) OR p.title LIKE ?) $p_order LIMIT 10");
...
if ($is_default_home) {
    ...
    // Prevent the same products from showing in both homepage sections.
    $recommended_ids = [];
