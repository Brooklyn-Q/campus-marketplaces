...
const categories = [
  ...
  { title: "Phone & Accessories",    image: `${BASE}IMG_5822.webp` },
  { title: "Electrical Appliances",  image: `${BASE}IMG_5827.webp` },
  { title: "Fashion",                image: `${BASE}IMG_5828.webp` },
  { title: "Food & Groceries",       image: `${BASE}IMG_5830.webp` },
  { title: "Education & Books",      image: `${BASE}IMG_5831.webp` },
  { title: "Hostels for Rent",       image: `${BASE}IMG_5833.webp` }
];
...
export default function Hero() {
  ...
  const handleSearch = () => {
    if (searchQuery.trim()) {
      window.location.href = `${BASE}index.php?search=${encodeURIComponent(searchQuery)}`;
    }
  };
    ...
    <>
      ...
      <div className="relative z-10 w-full pb-1">
        <div className="w-full">
          ...
          {/* Mobile scroll */}
          <div
            className="flex md:hidden overflow-x-auto gap-4 px-4 pb-8 snap-x snap-mandatory scroll-smooth w-full"
            style={{ WebkitOverflowScrolling: 'touch' } as any}
          >
            {categories.map((cat) => (
              <a
                key={cat.title}
                href={`${BASE}index.php?category=${encodeURIComponent(cat.title)}`}
