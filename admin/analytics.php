...
for ($i = 13; $i >= 0; $i--) {
    ...
    $dailyRevenue[] = ['label' => $label, 'value' => (float) ($revData[$date] ?? 0)];
    $dailyUsers[] = ['label' => $label, 'value' => (int) ($usrData[$date] ?? 0)];
}
            ...
            <tbody>
                <?php foreach ($topSellers as $i => $s): ?>
                    <tr>
                        ...
                        <td><span class="badge <?= $s['seller_tier'] === 'premium' ? 'badge-gold' : 'badge-blue' ?>"
                                style="font-size:0.6rem;"><?= ucfirst($s['seller_tier']) ?></span></td>
                        <td><?= $s['sale_count'] ?></td>
